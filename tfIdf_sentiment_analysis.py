...
def export_analysis_results(data: dict, ifAugment: bool):
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        for idx, (key, value) in enumerate(data.items()):
            ...
            f.write("\n情感判断: " + 
                  ("积极" if value['sentiment'] > 0.05 else 
                   "消极" if value['sentiment'] < -0.05 else "中性"))
            f.write("\n" + "-"*50)

        print("\n情感统计结果:")
        print(f"积极: {positive_count} 条 ({positive_count/total:.1%})")
        print(f"中性: {neutral_count} 条 ({neutral_count/total:.1%})")
        print(f"消极: {negative_count} 条 ({negative_count/total:.1%})")
